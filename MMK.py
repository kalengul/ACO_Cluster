...
#    @njit(parallel=True)
    ...
    if (pg.PG.typeProbability>=30) and (pg.PG.typeProbability<40):
        if (wayPg.pg.TypeKlaster >= 6000) and (wayPg.pg.TypeKlaster <= 6010):
            NameFileParetoSet=folderPg + '/EnableParetoSet/'+Setting.NameFileGraph[:-5]+str(Setting.KolParetto)+'_'+Model.Rosaviation.Rosaviation.column_index_rosaviation+'.xlsx'
        else:
